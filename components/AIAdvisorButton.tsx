...
const AIAdvisorButton: React.FC<AIAdvisorButtonProps> = ({ onClick, isActive = false }) => {
  return (
...
const StyledWrapper = styled.div<{ $isActive?: boolean }>`
  /* Right-align the button in its container */
  justify-self: end;
  ...
  .btn {
    ...
    background-color: ${props => props.$isActive ? '#0a0a0a' : 'var(--button-color)'};
  ...
  .btn-letter {
    position: relative;
    display: inline-block;
    color: ${props => props.$isActive ? '#ffffffcc' : '#fff5'};
  ...
  .btn-svg {
    ...
    width: 20px;
    margin-right: 0.4rem;
    fill: ${props => props.$isActive ? '#ffffffcc' : '#e8e8e8'};
